...
import dev.adventurecraft.awakening.natives.Bindings;
import dev.adventurecraft.awakening.natives.NativeLoader;
...
public abstract class MixinFrustumData implements ExFrustumData {

    @Unique protected final MemorySegment frustumPlanes = NativeLoader.arena.allocate(MemoryLayout.sequenceLayout(
        PLANES * DIMS,
    ...
    public boolean cubeInFrustum(double minX, double minY, double minZ, double maxX, double maxY, double maxZ) {
        return Bindings.ac_native_math_frustum_contains_cube(this.frustumPlanes, minX, minY, minZ, maxX, maxY, maxZ);
    }
